...
def Model(features, labels, mode, params):
    ...
    if EVAL:
        ...
        correct_prediction_20 = tf.reduce_mean(tf.cast(tf.equal(tf.cast(tf.tile(
            labels[:, 0:1], [1, 20]), tf.int32), tf.nn.top_k(logits[:, 0:1, :], 20)[1]), tf.float32)) * 20
        ...
        ndcg_20 = tf.reduce_mean(tf.math.log(2.0) / (tf.math.log(tf.cast(tf.where(tf.cast(tf.equal(tf.cast(tf.tile(labels[:, 0:1], [1, 1]), tf.int32), tf.nn.top_k(
            logits[:, 0:1, :], 20)[1]), tf.int64)), tf.float32) + 2.0))) / idcg_10  # * tf.cast(DEFINES.batch_size, tf.float32))

        ndcg_5 = (ndcg_5, ndcg_5)
        ndcg_10 = (ndcg_10, ndcg_10)
        ndcg_20 = (ndcg_20, ndcg_20)

        metrics = {'precision_5': precision_5, 'recall_5': recall_5,  'f1_5': f1_5, 'ndcg_5': ndcg_5, 'precision_10': precision_10, 'recall_10': recall_10,
                   'f1_10': f1_10, 'ndcg_10': ndcg_10,  'precision_20': precision_20, 'recall_20': recall_20,  'f1_20': f1_20, 'ndcg_20': ndcg_20}  # , 'pop_5':pop5, 'pop10':pop10}
        ...
        for word in sequence.split():
            ...
            if dictionary.get(word) is not None:
                ...
                value = euclidean_dist(cordinates[word], cordinates[pre_word]) if cordinates.get(
                    pre_word) is not None and cordinates.get(word) is not None else 1.0
...
def rearrange(input, in_distance, in_time, in_users, output, out_dist, out_t, out_users, target):
    features = {"input": input, "in_distance": in_distance, "in_time": in_time, "in_users": in_users,
                "output": output, "out_t": out_t, "out_distance": out_dist, "out_users": out_users}
...
def train_input_fn(train_input, train_out, train_target_dec, batch_size):
    ...
    (train_input_enc, train_input_dist,
     train_input_time, train_input_users) = train_input
    ...
    dataset = tf.data.Dataset.from_tensor_slices((train_input_enc, train_input_dist, train_input_time, train_input_users,
                                                 train_output_dec, train_output_dist, train_output_time, train_output_users, train_target_dec))
    # Decay entire data
...
def eval_input_fn(eval_input, eval_out, eval_target_dec, batch_size):
    ...
    # Divide the three into one sentence each.
    dataset = tf.data.Dataset.from_tensor_slices((eval_input_enc, eval_input_dist, eval_input_time,
                                                 eval_input_users, eval_output_dec, eval_output_dist, eval_output_time, eval_out_users, eval_target_dec))
    # Shuffle entire data
...
def main(dataset):
    ...
    results = pd.DataFrame(columns=['pre_5', 'pre_10', 'f1_5', 'f1_10', 'recall_5',
                           'recall_10', 'ndcg_5', 'ndcg_10', 'pop_5', 'dis_5', 'pop_10', 'dis_10'])
    iteration_number = 10
    ...
    train_input = (train_input_enc, train_input_dist,
                   train_input_time, train_input_users)
    ...
    eval_input = (eval_input_enc, eval_input_dist,
                  eval_input_time, eval_input_users)
    ...
    for i in range(iteration_number):
        ...
        classifier.train(
            input_fn=lambda: train_input_fn(
                train_input, train_output, train_target_dec, DEFINES.batch_size),
        ...
        # (prediction_value, eval_input_enc, idx2char,POPULARITY)
    ...
    print(" Average results = ", pre_5, recall_5, f1_5, ndcg_5, pre_10,
          recall_10, f1_10, ndcg_10, pre_20, recall_20, f1_20, ndcg_20)
...
if __name__ == '__main__':
    ...
    datasets = ['Pert']
    results = pd.DataFrame(columns=['dataset', 'pre_5', 'recall_5', 'f1_5', 'ndcg_5',
                           'pre_10', 'recall_10', 'f1_10', 'ndcg_10', 'pre_20', 'recall_20', 'f1_20', 'ndcg_20'])
    ...
    results.to_excel("Results_Final_DMKD/TLR-DRLIR_results.xlsx")
