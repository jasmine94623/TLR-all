...
dataset = 'Pert'  # Melbourne, Edin, Buda, caliAdv, MagicK, epcot
...
def evaluation_distance(value, targets, idx2char, cordinates):
    ...
    for v in value:
        ...
        for x in top_k[0:5]:
            dis_5 += euclidean_dist(cordinates[idx2char[x]],
                                    cordinates[idx2char[target_poi]])

        for x in top_k:
            dis_10 += euclidean_dist(cordinates[idx2char[x]],
                                     cordinates[idx2char[target_poi]])
...
def rearrange(input, in_distance, in_time, in_queue, in_users, output, out_dist, out_t, out_queue, out_users, target, target_queue):
    features = {"input": input, "in_distance": in_distance, "in_time": in_time, "in_queue": in_queue, "in_users": in_users,
                "output": output, "out_t": out_t, "out_distance": out_dist, "out_queue": out_queue, "out_users": out_users}
...
def train_input_fn(train_input, train_out, train_target_dec, train_target_queue, batch_size):
    ...
    (train_input_enc, train_input_dist, train_input_time,
     train_input_users, train_input_queue) = train_input
    ...
    dataset = tf.data.Dataset.from_tensor_slices((train_input_enc, train_input_dist, train_input_time, train_input_queue, train_input_users,
                                                 train_output_dec, train_output_dist, train_output_time, train_output_queue, train_output_users, train_target_dec, train_target_queue))
    # Decay entire data
...
def eval_input_fn(eval_input, eval_out, eval_target_dec, eval_target_queue, batch_size):
    (eval_input_enc, eval_input_dist, eval_input_time,
     eval_input_users, eval_input_queue) = eval_input
    ...
    # Divide the three into one sentence each.
    dataset = tf.data.Dataset.from_tensor_slices((eval_input_enc, eval_input_dist, eval_input_time, eval_input_queue, eval_input_users,
                                                 eval_output_dec, eval_output_dist, eval_output_time, eval_output_queue, eval_out_users, eval_target_dec, eval_target_queue))
    # Shuffle entire data
...
if __name__ == '__main__':
    ...
    for i in range(iteration_number):
        ...
        train_input = (train_input_enc, train_input_dist,
                       train_input_time, train_input_users, train_input_queue)
        train_output = (train_output_dec, train_output_dist,
                        train_output_time, train_output_users, train_output_queue)

        eval_input = (eval_input_enc, eval_input_dist,
                      eval_input_time, eval_input_users, eval_input_queue)
        ...
        #
        # evaluation_popularity(prediction_value, eval_input_enc, idx2char, POPULARITY)
        popularity_5, popularity_10, distance_5, distance_10 = 0.0, 0.0, 0.0, 0.0
        # print("Iteration = ", i, "  Popularities = ", popularity_5, popularity_10, distance_5, distance_10)

        prediction_value = classifier.predict(input_fn=lambda: eval_input_fn(
            eval_input, eval_output, eval_target_dec, eval_target_queue, DEFINES.batch_size))
        print(prediction_value)
        distance_5, distance_10 = evaluation_distance(
            prediction_value, eval_input_enc, idx2char, Cordinates)
    ...
    results.to_excel('Results_Final_DMKD/TMLR_results_' +
                     dataset+"_2_" + str(loss_alpha)+'pop.xlsx')
