...
def feed_forward(inputs, num_units):
    ...
    with tf.compat.v1.variable_scope("feed_forward", reuse=tf.compat.v1.AUTO_REUSE):
        outputs = tf.keras.layers.Dense(
            num_units[0], activation=tf.nn.relu)(inputs)
...
def scaled_dot_product_attention(query, key, value, masked=False):
    ...
    if masked:
        ...
        masks = tf.compat.v1.tile(tf.expand_dims(
            tril, 0), [tf.shape(outputs)[0], 1, 1])
...
def multi_head_attention(query, key, value, heads, masked=False):
    ...
    with tf.compat.v1.variable_scope("multi_head_attention", reuse=tf.compat.v1.AUTO_REUSE):
        ...
        attn_outputs = tf.concat(
            tf.split(attention_map, heads, axis=0), axis=-1)
...
def decoder_module(inputs, encoder_outputs, num_units, heads):
    ...
    masked_self_attn = sublayer_connection(
        inputs, multi_head_attention(inputs, inputs, inputs, heads, masked=True))

    self_attn = sublayer_connection(masked_self_attn, multi_head_attention(
        masked_self_attn, encoder_outputs, encoder_outputs, heads))
...
def decoder2(inputs, inputs2, encoder_outputs, encoder_outputs_queue, num_units, heads, num_layers):
    ...
    for _ in range(num_layers):
        outputs = decoder_module(outputs, encoder_outputs, num_units, heads)
        outputs2 = decoder_module(
            outputs2, encoder_outputs_queue, num_units, heads)
        outputs = outputs + outputs2
...
def Model(features, labels, mode, params):
    ...
    positional_encoded = positional_encoding(
        params['embedding_size'], DEFINES.max_sequence_length)
    positional_encoded.trainable = True
    if TRAIN:
        position_inputs = tf.tile(
            tf.range(0, DEFINES.max_sequence_length), [DEFINES.batch_size])
        position_inputs = tf.reshape(
            position_inputs, [DEFINES.batch_size, DEFINES.max_sequence_length])
    else:
        position_inputs = tf.tile(
            tf.range(0, DEFINES.max_sequence_length), [1])
        position_inputs = tf.reshape(
            position_inputs, [1, DEFINES.max_sequence_length])

    embedding = tf.compat.v1.get_variable(name='embedding', shape=[
                                          params['vocabulary_length'], params['embedding_size']], dtype=tf.float32, initializer=tf.initializers.GlorotUniform())  # tf.layers.xavier_initializer())
    W = tf.compat.v1.tile(tf.expand_dims(tf.compat.v1.get_variable(
        'W', [1, params['embedding_size']], dtype=tf.float32), 0), [DEFINES.batch_size, 1, 1])
    embedding_user1 = tf.compat.v1.get_variable("embedding_user", shape=[
                                                params['user_length'], params['embedding_size']], dtype=tf.float32, initializer=tf.initializers.GlorotUniform())  # tf.contrib.layers.xavier_initializer())
    W_u = tf.tile(tf.expand_dims(tf.compat.v1.get_variable("W_u", [
                  params['embedding_size'], params['embedding_size']], dtype=tf.float32), 0), [DEFINES.batch_size, 1, 1])

    W_p = tf.tile(tf.expand_dims(tf.compat.v1.get_variable("W_p", [
                  params['embedding_size'], params['embedding_size']], dtype=tf.float32), 0), [DEFINES.batch_size, 1, 1])

    encoder_inputs = tf.matmul(tf.nn.embedding_lookup(
        embedding, features['input']), W_p)
    decoder_inputs = tf.matmul(tf.nn.embedding_lookup(
        embedding, features['output']), W_p)
    encoder_features = tf.matmul(tf.expand_dims(tf.cast(features['in_distance'], tf.float32), 2), W) + tf.matmul(
        tf.expand_dims(tf.cast(features['in_time'], tf.float32), 2), W)

    # * 0.5 + tf.matmul(tf.expand_dims(tf.cast(features['in_cate'], tf.float32), 2), W) * 0.5
    encoder_features_UI = tf.matmul(tf.expand_dims(
        tf.cast(features['in_poisim'], tf.float32), 2), W)

    encoder_user_features = tf.matmul(tf.nn.embedding_lookup(
        embedding_user1, features['in_users']), W_u)
    position_encode = tf.nn.embedding_lookup(
        positional_encoded, position_inputs)
    ...
    encoder_queue_features = tf.matmul(tf.expand_dims(
        tf.cast(features['in_queue'], tf.float32), 2), W)
    decoder_queue_features = tf.matmul(tf.expand_dims(
        tf.cast(features['out_queue'], tf.float32), 2), W)
    ...
    logits = tf.keras.layers.Dense(params['vocabulary_length'])(
        decoder_outputs + decoder_outputs_queue)

    predict = tf.argmax(logits[:, :, :], -1)
    logits_queue = tf.keras.layers.Dense(
        params['vocabulary_length'])(decoder_outputs_queue)

    # Update logits based on queue information
    logits = logits + logits_queue
    predict_queue = tf.nn.top_k(logits_queue[:, :, :], 1)[
        0]  # tf.argmax(logits_queue[:,:,:], -1)

    if PREDICT:
        predictions = {
            # tf.nn.top_k(logits[:,0:1,:],10)[1]
            'topk': tf.nn.top_k(logits[:, 0:1, :], 10)[1],
            'logit': logits,
    ...
    loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(
        logits=logits, labels=label1))
    loss_queue = tf.reduce_mean(tf.keras.losses.mean_squared_error(
        predict_queue, tf.cast(label2, tf.float32)))

    # tf.group(loss, loss_queue) # K.mean(loss + loss_queue)
    loss2 = loss_alpha * loss + (1 - loss_alpha) * loss_queue
    #loss2 = loss + loss_queue
    if EVAL:
        correct_prediction_5 = tf.reduce_mean(tf.cast(tf.equal(tf.cast(tf.tile(label1[:, 0:1], [1, 5]), tf.int32), tf.nn.top_k(logits[:, 0:1, :], 5)[
                                              1]), tf.float32))*5  # / tf.cast((tf.shape(logits)[0] * tf.shape(logits)[1]),tf.float32) #DEFINES.batch_size * params['vocabulary_length'])
        correct_prediction_10 = tf.reduce_mean(tf.cast(tf.equal(tf.cast(tf.tile(
            label1[:, 0:1], [1, 10]), tf.int32), tf.nn.top_k(logits[:, 0:1, :], 10)[1]), tf.float32)) * 10
        ...
        f1_5 = (2*recall_5[0]*precision_5[0] / (recall_5[0] + precision_5[0] + 1e-8),
                2*recall_5[0]*precision_5[0] / (recall_5[0] + precision_5[0] + 1e-8))
        f1_10 = (2 * recall_10[0] * precision_10[0] / (recall_10[0] + precision_10[0] + 1e-8),
                 2 * recall_10[0] * precision_10[0] / (recall_10[0] + precision_10[0] + 1e-8))

        idcg_5, idcg_10 = find_idcg()
        if idcg_5 != 0:
            ndcg_5 = tf.reduce_mean(tf.math.log(2.0) / (tf.math.log(tf.cast(tf.where(tf.cast(tf.equal(tf.cast(tf.tile(label1[:, 0:1], [1, 1]), tf.int32), tf.nn.top_k(
                logits[:, 0:1, :], 5)[1]), tf.int64)), tf.float32) + 2.0))) / idcg_5  # * tf.cast(DEFINES.batch_size, tf.float32))#
        else:
            ndcg_5 = 0.0
        if idcg_5 != 0:
            ndcg_10 = tf.reduce_mean(tf.math.log(2.0) / (tf.math.log(tf.cast(tf.where(tf.cast(tf.equal(tf.cast(tf.tile(label1[:, 0:1], [1, 1]), tf.int32), tf.nn.top_k(
                logits[:, 0:1, :], 10)[1]), tf.int64)), tf.float32) + 2.0))) / idcg_10  # * tf.cast(DEFINES.batch_size, tf.float32))
        else:
            ndcg_10 = 0.0
        ...
        rmse = tf.sqrt(tf.reduce_mean(tf.math.squared_difference(tf.cast(
            predict_queue * params['max_queue'], tf.float32), tf.cast(label2 * params['max_queue'], tf.float32))))
        rmse = (rmse, rmse)

        metrics = {'recall_5': recall_5, 'precision_5': precision_5, 'f1_5': f1_5, 'recall_10': recall_10,
                   'precision_10': precision_10, 'f1_10': f1_10, 'ndcg_5': ndcg_5, 'ndcg_10': ndcg_10, 'rmse': rmse}
    ...
    train_op1 = optimizer.minimize(
        loss_queue, global_step=tf.compat.v1.train.get_global_step())
...
def time_function(dataset, dfVisits, train_pois):
    ...
    dfVisit1 = dfVisits.sort_values(
        'takenUnix', ascending=True).drop_duplicates('poiID').reset_index()
    dfVisit2 = dfVisits.sort_values(
        'takenUnix', ascending=False).drop_duplicates('poiID').reset_index()
    ...
    for i in range(1, len(train_pois)):
        ...
        t = dfVisit1[dfVisit1.poiID == train_pois[i]].takenUnix.values[0] - \
            dfVisit1[dfVisit1.poiID == train_pois[i-1]].takenUnix.values[0] + 1
        hour = datetime.fromtimestamp(
            dfVisit1[dfVisit1.poiID == train_pois[i]].takenUnix.values[0]).hour
        if dataset in ['epcot', 'disland', 'MagicK', 'caliAdv', 'disHolly']:
            q = QTIME[(int(train_pois[i]), int(hour))]
        else:
            q = dfVisit1[dfVisit1.poiID == train_pois[i]].takenUnix.values[0] - \
                dfVisit2[dfVisit2.poiID == train_pois[i-1]
                         ].takenUnix.values[0] + 1
        #print("t = ", t , " q = ",q)
        train_queue += [q/60]
        train_time += [t/60]
...
def preprocess(dataset):
    ...
    dfVisits = pd.read_excel(
        'DataExcelFormat/userVisits-' + dataset + '-allPOI.xlsx')
    if dataset in ["epcot", "disland", "MagicK", "caliAdv", 'disHolly']:
        dfQueue = pd.read_excel('DataExcelFormat/queueTimes-'+dataset+'.xlsx')
        for i in range(dfQueue.shape[0]):
            poi_q, hour, t_q, = int(dfQueue.iloc[i]['poiID']), int(
                dfQueue.iloc[i]['hour']), dfQueue.iloc[i]['avgQueueTime']
    ...
    dfVisits['poi_freq'] = dfVisits.groupby(
        'poiID')['poiID'].transform('count')
    ...
    for seq in sequences:
        ...
        tempdfVisits = tempdfVisits.sort_values(
            ['takenUnix'], ascending=[True])
        ...
        for i in range(len(pois) - 1):
            ...
            df.at[df.shape[0]] = (train_d, test_d, userseq,
                                  train_time, train_queue, target_queue)
    ...
    for i in range(len(dfNodes)):
        ...
        for j in range(len(dfNodes)):
            if dfNodes.iloc[i].theme == dfNodes.iloc[j].theme:
                Category['POI_'+str(dfNodes.iloc[i].poiID),
                         'POI_'+str(dfNodes.iloc[j].poiID)] = 1
            else:
                Category['POI_'+str(dfNodes.iloc[i].poiID),
                         'POI_'+str(dfNodes.iloc[j].poiID)] = 0
...
def load_data(dataset):
    ...
    question, answer, user, time, queue, queue_target = list(data_df['Train']), list(
        data_df['Test']), list(data_df['U']), list(data_df['T']), list(data_df['Q']), list(data_df['Q_t'])
    random_seed = random.randint(10, 100)

    # print(random_seed)
    train_input, eval_input1, train_label, eval_label1 = train_test_split(
        question, answer, test_size=0.30, random_state=random_seed)
    train_user, eval_user = train_test_split(
        user,  test_size=0.30, random_state=random_seed)
    # devide time based on test size
    train_time, eval_time, train_queue, eval_queue = train_test_split(
        time, queue, test_size=0.30, random_state=random_seed)
    eval_input, test_input, eval_label, test_label = train_test_split(
        eval_input1, eval_label1, test_size=0.66, random_state=random_seed+1)
    train_target_queue, eval_target_queue = train_test_split(
        queue_target, test_size=0.30, random_state=random_seed)
...
def enc_processing(value, users, dictionary, cordinates):
    ...
    for user in users:
        ...
        sequence_user += (DEFINES.max_sequence_length -
                          len(sequence_user)) * [dictionary[PAD]]
    ...
    for sequence in value:
        ...
        for word in sequence.split():
            ...
            if dictionary.get(word) is not None:
                ...
                value = euclidean_dist(cordinates[word], cordinates[pre_word]) if cordinates.get(
                    pre_word) is not None else 1.0
        ...
        sequence_index += (DEFINES.max_sequence_length -
                           len(sequence_index)) * [dictionary[PAD]]
        sequence_distance += (DEFINES.max_sequence_length -
                              len(sequence_distance))*[dictionary[PAD]]
    ...
    sequence_input_distance = np.asarray(
        [[y / max_distance for y in x] for x in sequence_input_distance])
...
def rearrange(input, in_distance, in_time, in_queue, in_users, in_cate, in_poisim, output, out_dist, out_t, out_queue, out_users, target, target_queue):
    features = {"input": input, "in_distance": in_distance, "in_time": in_time, "in_queue": in_queue, "in_users": in_users, "in_cate": in_cate,
                "in_poisim": in_poisim, "output": output, "out_t": out_t, "out_distance": out_dist, "out_queue": out_queue, "out_users": out_users}
...
def train_input_fn(train_input, train_out, train_target_dec, train_target_queue, batch_size):
    ...
    (train_input_enc, train_input_dist, train_input_time, train_input_users,
     train_input_queue, train_input_cate, train_input_poisim) = train_input
    (train_output_dec, train_output_dist, train_output_time,
     train_output_users, train_output_queue) = train_out
    ...
    dataset = tf.data.Dataset.from_tensor_slices((train_input_enc, train_input_dist, train_input_time, train_input_queue, train_input_users, train_input_cate,
                                                 train_input_poisim, train_output_dec, train_output_dist, train_output_time, train_output_queue, train_output_users, train_target_dec, train_target_queue))
    # Decay entire data
...
def eval_input_fn(eval_input, eval_out, eval_target_dec, eval_target_queue, batch_size):
    (eval_input_enc, eval_input_dist, eval_input_time, eval_input_users,
     eval_input_queue, eval_input_cate, eval_input_poisim) = eval_input
    (eval_output_dec, eval_output_dist, eval_output_time,
     eval_out_users, eval_output_queue) = eval_out

    # As part of creating dataset, from_tensor_slices part
    # Cut each sentence into one sentence.
    # eval_input_enc, eval_output_dec, eval_target_dec
    # Divide the three into one sentence each.
    dataset = tf.data.Dataset.from_tensor_slices((eval_input_enc, eval_input_dist, eval_input_time, eval_input_queue, eval_input_users, eval_input_cate,
                                                 eval_input_poisim, eval_output_dec, eval_output_dist, eval_output_time, eval_output_queue, eval_out_users, eval_target_dec, eval_target_queue))
    # Shuffle entire data
...
def makeDoc2VecData(dataset):
    descriptoin = pd.read_excel(
        'DataExcelFormat/POI-' + dataset + 'withDescription.xlsx')
    questionForm = pd.DataFrame(
    ...
    for i in range(len(descriptoin)):
        for j in range(i, len(descriptoin)):
            if descriptoin.iloc[i].theme == descriptoin.iloc[j].theme:
                questionForm.at[questionForm.shape[0]] = [index, descriptoin.iloc[i].poiID,
                                                          descriptoin.iloc[j].poiID, descriptoin.iloc[i].description, descriptoin.iloc[j].description, 1]
            else:
                questionForm.at[questionForm.shape[0]] = [index, descriptoin.iloc[i].poiID,
                                                          descriptoin.iloc[j].poiID, descriptoin.iloc[i].description, descriptoin.iloc[j].description, 0]
        index = index + 1
    return questionForm
    ...
    for i in range(input.shape[0]):
        ...
        for j in range(1, input.shape[1]):
            if input[i][j] != 0:
                ...
                new_S[i, j] = S[idx2char[input[i, j - 1]], idx2char[input[i, j]]] if (
                    idx2char[input[i, j - 1]], idx2char[input[i, j]]) in S else S[idx2char[input[i, j]], idx2char[input[i, j-1]]]
...
def main(dataset):
    ...
    results = pd.DataFrame(columns=[
                           'pre_5', 'pre_10', 'f1_5', 'f1_10', 'recall_5', 'recall_10', 'ndcg_5', 'ndcg_10', 'rmse'])
    ...
    train_input_enc, train_input_dist, train_input_users, train_input_enc_length = enc_processing(
        train_input, train_user, char2idx, Cordinates)
    ...
    eval_input_enc, eval_input_dist, eval_input_users, eval_input_enc_length = enc_processing(
        eval_input, eval_user, char2idx, Cordinates)
    ...
    user_length = max(max([max(y) for y in eval_input_users]), max(
        [max(y) for y in train_input_users]))+1
    # Find max queue time and normalize based on 0-1
    max_queue = max(max([max(y) for y in train_input_queue]),
                    max([max(y) for y in eval_input_queue]))
    # print(max_queue)

    train_input_queue = np.asarray(
        [[y / max_queue for y in x] for x in train_input_queue])
    eval_input_queue = np.asarray(
        [[y / max_queue for y in x] for x in eval_input_queue])
    train_output_queue = np.asarray(
        [[y / max_queue for y in x] for x in train_output_queue])
    eval_output_queue = np.asarray(
        [[y / max_queue for y in x] for x in eval_output_queue])
    # print(train_target_queue)
    # print(eval_target_queue)
    train_target_queue = np.asarray(
        [y / max_queue for y in train_target_queue])
    eval_target_queue = np.asarray([y / max_queue for y in eval_target_queue])

    DEFINES.max_queue_time = max_queue

    cate_train_enc, train_POI_similarity = category_paragraph_sequence(
        train_input_enc, Category, POI_des_sim, idx2char)
    cate_eval_enc, eval_POI_similarity = category_paragraph_sequence(
        eval_input_enc, Category, POI_des_sim, idx2char)
    ...
    for i in range(iteration_number):
        ...
        train_input = (train_input_enc, train_input_dist, train_input_time,
                       train_input_users, train_input_queue, cate_train_enc, train_POI_similarity)
        train_output = (train_output_dec, train_output_dist,
                        train_output_time, train_output_users, train_output_queue)

        eval_input = (eval_input_enc, eval_input_dist, eval_input_time,
                      eval_input_users, eval_input_queue, cate_eval_enc, eval_POI_similarity)
        eval_output = (eval_output_dec, eval_output_dist,
                       eval_output_time, eval_output_users, eval_output_queue)

        start_training_time = timeit.default_timer()
        classifier.train(input_fn=lambda: train_input_fn(train_input, train_output,
                         train_target_dec, train_target_queue, DEFINES.batch_size), steps=DEFINES.train_steps)
        train_execution_time = train_execution_time + \
            timeit.default_timer()-start_training_time

        start_test_time = timeit.default_timer()
        eval_result = classifier.evaluate(input_fn=lambda: eval_input_fn(
            eval_input, eval_output, eval_target_dec, eval_target_queue, DEFINES.batch_size), steps=1)
        test_execution_time = test_execution_time + \
            timeit.default_timer() - start_test_time
        print('\nEVAL set precision_5:{precision_5:0.3f} recall_5:{recall_5:0.3f} precision_10:{precision_10:0.3f} recall_10:{recall_10:0.3f}  ndcg_5:{ndcg_5:0.3f}\n ndcg_10:{ndcg_10:0.3f}'.format(
            **eval_result))

        print("Iterations = ", i + 1)
        c_pre_5, c_recall_5, c_f1_5, c_pre_10, c_recall_10, c_f1_10, c_ndcg_5, c_ndcg_10, rmse = '{precision_5:0.6f},{recall_5:0.6f}, {f1_5:0.6f}, {precision_10:0.6f},{recall_10:0.6f},{f1_10:0.6f}, {ndcg_5:0.6f},{ndcg_10:0.6f}, {rmse:0.6f}'.format(
            **eval_result).split(",")
        print(c_pre_5, c_recall_5, c_f1_5, c_pre_10,
              c_recall_10, c_f1_10, c_ndcg_5, c_ndcg_10, rmse)
        ...
        results.at[results.shape[0]] = [c_pre_5, c_pre_10, c_f1_5,
                                        c_f1_10, c_recall_5, c_recall_10, c_ndcg_5, c_ndcg_10, rmse]
    ...
    print("The test data total_precison_5 is %f, total_precison_10 is %f" %
          (pre_5, pre_10))
    print("The test data total_recall_5 is %f, total_recall_10 is %f" %
          (recall_5, recall_10))
    print("The test data total_f1_5 is %f, total_f1_10 is %f" % (f1_5, f1_10))
    print("The test data total_ncdd_5 is %f, total_ndcg_10 is %f" %
          (ndcg_5, ndcg_10))
    ...
    if (DEFINES.user_interest == 1):
        results.to_excel(
            'Results_Final_DMKD/TLRM-UI_results_POIDes' + dataset+'.xlsx')
    else:
        results.to_excel(
            'Results_Final_DMKD/TLRM-UI_results_Cate' + dataset + '.xlsx')
...
if __name__ == '__main__':
    ...
    executive_time = pd.DataFrame(
        columns=['Dataset', 'total_time', 'train_time', 'test_time'])
    ...
    for data in datasets:
        ...
        print("Time Duration = ", ex_time, " seconds",
              "Training time = ", train_time, " test time = ", test_time)
        executive_time.at[executive_time.shape[0]] = [
            data, ex_time, train_time, test_time]
    if (DEFINES.user_interest == 1):
