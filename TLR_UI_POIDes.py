...
def Model(features, labels, mode, params):
    ...
    # , initializer=tf.compat.layers.xavier_initializer())
    embedding = tf.compat.v1.get_variable(name='embedding', shape=[
                                          params['vocabulary_length'], params['embedding_size']], dtype=tf.float32)
    ...
    embedding_user1 = tf.compat.v1.get_variable("embedding_user", shape=[
                                                params['user_length'], params['embedding_size']], dtype=tf.float32)  # initializer=tf.contrib.layers.xavier_initializer())
    ...
    # + tf.matmul(tf.expand_dims(tf.cast(features['in_cate'], tf.float32), 2), W)
    encoder_UI_features = tf.matmul(tf.expand_dims(
        tf.cast(features['in_poisim'], tf.float32), 2), W)

    # tf.matmul(tf.expand_dims(tf.cast(features['in_users'], tf.float32), 2), W)
    ...
    encoder_outputs = encoder(encoder_inputs,
                              [params['hidden_size'] * 4,
                                  params['hidden_size']], DEFINES.heads_size,
                              DEFINES.layers_size)
    ...
    logits = tf.keras.layers.Dense(
        params['vocabulary_length'])(decoder_outputs)
    ...
    if DEFINES.mask_loss:
        embedding_tile = tf.tile(tf.expand_dims(embedding, 0), [
                                 DEFINES.batch_size, 1, 1])
        linear_outputs = tf.matmul(
            decoder_outputs, embedding_tile, transpose_b=True)

        #mask_zero = 1 - tf.cast(tf.equal(labels, 0),dtype=tf.float32)
        mask_end = 1 - tf.cast(tf.equal(labels, 2), dtype=tf.float32)
        # [BS, senxlen, vocab_size]
        labels_one_hot = tf.one_hot(
            indices=labels, depth=params['vocabulary_length'], dtype=tf.float32)
        loss = tf.nn.softmax_cross_entropy_with_logits(
            labels=labels_one_hot, logits=linear_outputs)
        #loss = loss * mask_zero
        loss = loss * mask_end
    ...
    if EVAL:
        correct_prediction_5 = tf.reduce_mean(tf.cast(tf.equal(tf.cast(tf.tile(labels[:, 0:1], [1, 5]), tf.int32), tf.nn.top_k(logits[:, 0:1, :], 5)[
        ...
        correct_prediction_10 = tf.reduce_mean(tf.cast(tf.equal(tf.cast(tf.tile(
            labels[:, 0:1], [1, 10]), tf.int32), tf.nn.top_k(logits[:, 0:1, :], 10)[1]), tf.float32)) * 10
        ...
        ndcg_5 = tf.reduce_mean(tf.math.log(2.0) / (tf.math.log(tf.cast(tf.where(tf.cast(tf.equal(tf.cast(tf.tile(labels[:, 0:1], [1, 1]), tf.int32), tf.nn.top_k(
            logits[:, 0:1, :], 5)[1]), tf.int64)), tf.float32) + 2.0))) / idcg_5  # * tf.cast(DEFINES.batch_size, tf.float32))#
        ndcg_10 = tf.reduce_mean(tf.math.log(2.0) / (tf.math.log(tf.cast(tf.where(tf.cast(tf.equal(tf.cast(tf.tile(labels[:, 0:1], [1, 1]), tf.int32), tf.nn.top_k(
        ...
        metrics = {'recall_5': recall_5, 'precision_5': precision_5, 'f1_5': f1_5, 'recall_10': recall_10,
                   'precision_10': precision_10, 'f1_10': f1_10, 'ndcg_5': ndcg_5, 'ndcg_10': ndcg_10}  # , 'pop_5':pop5, 'pop10':pop10}
...
def time_function(dfVisits, train_pois):
    ...
    for i in range(1, len(train_pois)):
        t = (dfVisit1[dfVisit1.poiID == train_pois[i]].takenUnix.values[0] -
             dfVisit1[dfVisit1.poiID == train_pois[i-1]].takenUnix.values[0] + 1)//60
...
def preprocess(dataset):
    ...
    # Find datasets based on POI_numbers
    poiNumber = min(max(dfVisits.poiID), DEFINES.POI_numbers)
    ...
    for seq in sequences:
        ...
        max_len = max(len(pois)*train_part+0.5, len(pois)*(1-train_part) + 0.5) if max_len < max(
            len(pois)*train_part+0.5, len(pois)*(1-train_part) + 0.5) else max_len
...
def load_data(dataset, time):
    ...
    question, answer, user, timeid = list(data_df['Q']), list(
        data_df['A']), list(data_df['U']), list(data_df['T'])
    ...
    train_user, eval_user, train_timeid, eval_timeid = train_test_split(
        user, timeid, test_size=0.30, random_state=random_seed)
    # devide time based on test size
    train_time, eval_time = train_test_split(
        time, test_size=0.30, random_state=random_seed)
...
def rearrange(input, in_distance, in_time, in_users, in_cate, in_poisim, output, out_dist, out_t, out_users, target):
    features = {"input": input, "in_distance": in_distance, "in_time": in_time, "in_users": in_users, "in_cate": in_cate,
                "in_poisim": in_poisim, "output": output, "out_t": out_t, "out_distance": out_dist, "out_users": out_users}
...
def train_input_fn(train_input, train_out, train_target_dec, batch_size):
    ...
    (train_input_enc, train_input_dist, train_input_time,
     train_input_users, train_input_cate, train_input_poisim) = train_input
    (train_output_dec, train_output_dist,
     train_output_time, train_output_users) = train_out
    ...
    dataset = tf.data.Dataset.from_tensor_slices((train_input_enc, train_input_dist, train_input_time, train_input_users, train_input_cate,
                                                 train_input_poisim, train_output_dec, train_output_dist, train_output_time, train_output_users, train_target_dec))
    # Decay entire data
...
def eval_input_fn(eval_input, eval_out, eval_target_dec, batch_size):
    (eval_input_enc, eval_input_dist, eval_input_time,
     eval_input_users, eval_input_cate, eval_input_poisim) = eval_input
    ...
    # Divide the three into one sentence each.
    dataset = tf.data.Dataset.from_tensor_slices((eval_input_enc, eval_input_dist, eval_input_time, eval_input_users, eval_input_cate,
                                                 eval_input_poisim, eval_output_dec, eval_output_dist, eval_output_time, eval_out_users, eval_target_dec))
    # Shuffle entire data
...
def makeDoc2VecData(dataset):
    if dataset in "Gowall,Foursquare":
        descriptoin = pd.read_excel(
            'DataExcelFormat/POI-" + dataset +"_"+str(POI_number)+ ".xlsx')
    else:
        descriptoin = pd.read_excel(
...
def main(dataset):
    ...
    results = pd.DataFrame(columns=[
                           'pre_5', 'pre_10', 'f1_5', 'f1_10', 'recall_5', 'recall_10', 'ndcg_5', 'ndcg_10'])
    iteration_number = 10
    ...
    train_input, train_label, eval_input, eval_label, test_input, test_label, train_time, eval_time, user, timeid = load_data(
        dataset, time)
    ...
    cate_input_enc, train_POI_similarity = category_paragraph_sequence(
    ...
    for i in range(iteration_number):
        ...
        train_input = (train_input_enc, train_input_dist, train_input_time,
                       train_input_users, cate_input_enc, train_POI_similarity)
        train_output = (train_output_dec, train_output_dist,
                        train_output_time, train_output_users)

        eval_input = (eval_input_enc, eval_input_dist, eval_input_time,
                      eval_input_users, cate_eval_enc, eval_POI_similarity)
        eval_output = (eval_output_dec, eval_output_dist,
                       eval_output_time, eval_output_users)

        start_training_time = timeit.default_timer()
        classifier.train(input_fn=lambda: train_input_fn(
            train_input, train_output, train_target_dec, DEFINES.batch_size), steps=DEFINES.train_steps)
        ...
        eval_result = classifier.evaluate(input_fn=lambda: eval_input_fn(
            eval_input, eval_output, eval_target_dec, DEFINES.batch_size), steps=1)
        ...
        results.at[results.shape[0]] = [c_pre_5, c_pre_10, c_f1_5,
                                        c_f1_10, c_recall_5, c_recall_10, c_ndcg_5, c_ndcg_10]
    ...
    if (DEFINES.user_interest == 1):
        results.to_excel(
            'Results_Final_DMKD/TLR-UI_results_POIDes' + dataset + '.xlsx')
    else:
        results.to_excel(
            'Results_Final_DMKD/TLR-UI_results_Cate' + dataset + '.xlsx')
...
if __name__ == '__main__':
    ...
    if (DEFINES.user_interest == 1):
        executive_time.to_excel(
            'Results_Final_DMKD/TLR-UI_POIDes_Executive_time.xlsx')
    else:
        executive_time.to_excel(
            'Results_Final_DMKD/TLR-UI_Cate_Executive_time.xlsx')
